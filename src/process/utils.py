...
def load_code_descriptions(version='mimic3'):
    ...
    if version == 'mimic2':
        ...
        with open("%s/D_ICD_DIAGNOSES.csv" % (data_folder), 'r') as descfile:
        ...
        with open("%s/D_ICD_PROCEDURES.csv" % (data_folder), 'r') as descfile:
            r = csv.reader(descfile)
            #header
            next(r)
            for row in r:
        ...
        with open('%s/ICD9_descriptions' % data_folder, 'r') as labelfile:
